...
public class EntityUuidArgumentType implements ArgumentType<Entity> {
    ...
    public Entity parse(StringReader reader) throws CommandSyntaxException {
        ...
        for (var entity : mc.world.getEntities()) {
            if (argument.equals(entity.getName().getString())) return entity;
        }
    ...
    public <S> CompletableFuture<Suggestions> listSuggestions(CommandContext<S> context, SuggestionsBuilder builder) {
        ...
        for (var entity : mc.world.getEntities()) {
            possibleSuggestions.add(entity.getName().getString());
        }
    ...
    public Collection<String> getExamples() {
        return List.of(mc.player.getName().getString(), mc.player.getUuidAsString());
    }
}
